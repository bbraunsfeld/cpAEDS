...
from cpaeds.utils import (load_config_yaml,get_dir_list,write_file,read_energyfile,read_state_file,read_df,plot_offset_ratio,plot_offst_dG,
                        check_finished,write_file2,read_output,plot_offset_pH,plot_offset_pH_fraction,density_plot,state_density_csv,kde_ridge_plot,density_map_to_concat_df)
...
"""
_summary_
Script to analyse a continues runs. Concat all e1,e2,e2r and vmix files.
Returns:
                ...
                for i, dir in enumerate(dir_list):
                        if dir == 'results':
                                ...
                                if run_finished == True:
                                        ...
                                        exe = subprocess.run(
                                        ['/local/gromosPlsPls/gromos++/BUILD/bin/ene_ana', '@f', 'ene_ana.arg'],
                                        check=True,
                                        capture_output=True,
                                        text=True
                                        )
                                        ...
                                        print('Running ene_ana for unfinished run...')
                                        exe = subprocess.run(
                                        ['/local/gromosPlsPls/gromos++/BUILD/bin/ene_ana', '@f', 'ene_ana_temp.arg'],
                                        check=True,
                                        capture_output=True,
                                        text=True
                                        )
                                ...
                                with open('df.out', 'w') as sp:
                                        exe = subprocess.run(
                                        ['/local/gromosPlsPls/gromos++/BUILD/bin/dfmult', '@f', 'df.arg'], 
                                        stdout=sp)
                                ...
                                density_map_e1r.append([e1r,t1r])
                                density_map_e2r.append([e2r,t2r])
